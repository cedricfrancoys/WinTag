/* eventlistener.cpp - Singleton implementation of a global event listener.
...
LRESULT EventListener::handle(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CONTEXTMENU:
		...
		break;
	case WM_NOTIFY:
		msg = ((LPNMHDR)lParam)->code;
		itemId = ((LPNMHDR)lParam)->idFrom;
		itemHWnd = ((LPNMHDR)lParam)->hwndFrom;
		break;
	}
	// search for a matching node
